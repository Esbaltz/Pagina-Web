function form1(){
document.querySelector(".agregar_pro_form").addEventListener("submit", function (event) {
    ...
    if (nombre_libr.length < 3 || nombre_libr.length > 60) {
        nombre_libro_ingre_error.innerHTML = "El nombre del libro ingresado tiene que tener un paramentro minimo de 3 caracteres o un maximo de 60.";
        event.preventDefault();
    }
    if (isbn.length !== 13) {
        nombre_isbn_error_1.innerHTML = "Los datos de ISBN no tiene el largo de 13 caracteres.";
        event.preventDefault();
    }
    ...
    if ((valor_libr.toString()).length < 2 || (valor_libr.toString()).length > 7) {
        valor_libro_error.innerHTML = "El valor del libro ingresado tiene que tener un paramentro minimo de 3 cifras, un maximo de 7 o no puede ingresar el valor 0 en solo.";
        event.preventDefault();
    }
    if (valor_libr < 1) {
...
document.querySelector(".buscar_pro_form").addEventListener("submit", function(event){
    ...
    var nombre_libro_ingre_error = document.getElementById("nombre_libro_ingre_error1");
    ...
    if(nombre_libr.length < 3 || nombre_libr.length > 60){
        nombre_libro_ingre_error.innerHTML = "El nombre del libro ingresado debe tener entre 3 y 60 caracteres.";
        event.preventDefault();
    }
    ...
    if(descrip.length < 10 || descrip.length > 600){
        descrip_error_libr.innerHTML = "Los detalles del libro deben tener entre 10 y 600 caracteres.";
        event.preventDefault(); 
    }
});
}
...
function crear_c(){
    document.getElementById("crear_cuenta_Form").addEventListener("submit", function (event) {
        var nombreCuenta = document.getElementById("nombreCuenta").value;
        var password_1 = document.getElementById("password_1").value;
        var confirmPassword = document.getElementById("confirmPassword").value;


        var nombreCuentaError = document.getElementById("nombre_libro_ingre_error");
        var passwordError = document.getElementById("passwordError");
        var passwordError_confirm = document.getElementById("passwordError_confirm");
...
function elim(){
    document.getElementById("agregar_pro_form").addEventListener("submit", function (event) {
        var isbn = document.getElementById("isbn").value;
